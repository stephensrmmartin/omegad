...
parameters {
  ...
  // GP
  row_vector[N_loadings] gp_linear_raw;
  // row_vector[F] gp_linear;
  vector<lower=0>[N_loadings] gp_rho_raw;
  // vector<lower=0>[F] gp_rho;
  vector<lower=0>[N_loadings] gp_alpha_raw;
  // vector<lower=0>[F] gp_alpha;
  matrix[M,N_loadings] gp_z_raw;
...
transformed parameters {
  ...
  // Unroll lambda_loc and GP parameters.
  {
    int count = 1;
    for(f in 1:F){
      lambda_loc_mat[f,F_inds[f,1:F_inds_num[f]]] = lambda_loc[count:(count - 1 + F_inds_num[f])];
      gp_linear[f,F_inds[f,1:F_inds_num[f]]] = gp_linear_raw[count:(count - 1 + F_inds_num[f])];
      gp_rho[f,F_inds[f,1:F_inds_num[f]]] = gp_rho_raw[count:(count - 1 + F_inds_num[f])];
      gp_alpha[f,F_inds[f,1:F_inds_num[f]]] = gp_alpha_raw[count:(count - 1 + F_inds_num[f])];
      gp_z[f,,F_inds[f,1:F_inds_num[f]]] = gp_z_raw[, count:(count - 1 + F_inds_num[f])];
      count += F_inds_num[f];
    }
  ...
  shat = rep_matrix(nu_sca,N); //NOT EXPONENTIATED
...
model {
  ...
  /* GP */
  to_vector(gp_linear) ~ std_normal();
  for(f in 1:F) {
    to_vector(gp_z[f]) ~ std_normal();
  }
  to_vector(gp_alpha) ~ student_t(3,0,2);
  to_vector(gp_rho) ~ std_normal();
