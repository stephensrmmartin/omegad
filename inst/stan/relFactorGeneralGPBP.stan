functions {
  ...
  vector spd_gp_fast(matrix x_phi, real alpha, real rho, vector lambdas,
                     vector gp_z) {
    int M = cols(x_phi);
    vector[M] spdsOut = spds(alpha, rho, lambdas);
...
transformed data {
  ...
  array[P - 1] matrix[N, M] gp_exo_phi;
...
parameters {
  ...
  array[P - 1] matrix[M, F] exo_gp_z;
}
transformed parameters {
  matrix[N, F] theta_loc = theta_loc_z * theta_cor_L';
  array[F] matrix[N, M] gp_theta_phi;
  ...
  for (f in 1 : F) {
    ...
    theta_sca[ : , f] += spd_gp_fast(gp_theta_phi[f], gp_alpha[f], gp_rho[f],
                                     gp_lambdas, gp_z[ : , f]); // SPD-BF-RBF
  }
  /* Exogenous GP */
  for (p in 1 : (P - 1)) {
    for (f in 1 : F) {
      theta_sca[ : , f] += exo_x[ : , p + 1] * exo_gp_linear[p, f]; // Linear
      theta_sca[ : , f] += spd_gp_fast(gp_exo_phi[p], exo_gp_alpha[p, f],
                                       exo_gp_rho[p, f], gp_lambdas,
                                       exo_gp_z[p, 1 : M, f]);
    }
  }
